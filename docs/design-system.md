...
The canonical UI design system for the active `frontend-app/` (Capacitor + PWA)
client. Extracted from the try-this-mockups (18 screens). This supersedes the
older Expo theme; the forest/ink/linen palette and type scale below are the
source of truth — do not hardcode colors or font sizes, use these tokens.
