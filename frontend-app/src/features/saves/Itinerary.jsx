...
export default function Itinerary({ onNavigate, onBack, payload }) {
  ...
  // The shared message is the plan itself: each day, each stop with its time
  // and how long it takes, the stays and the way there. Read on WhatsApp it
  // should need no app and no link.
  ...
  const planText = () => {
    ...
    const day = (d) => {
      const stops = (d.stops || []).map((x, i) => {
        const when = hoursAt(i, d.stops);
        const how = x.durationHr ? ` (${x.durationHr === Math.round(x.durationHr) ? x.durationHr : x.durationHr.toFixed(1)}h)` : '';
        return `  ${when}${how ? ` ${how}` : ''}  ${x.place}${x.notes ? ` — ${x.notes}` : ''}`;
      }).join('\n');
      const head = `Day ${d.day} — ${d.theme || ''}`.trim() + (d.stayArea ? ` · stay in ${d.stayArea}` : '') + (d.travelTimeTotalHr ? ` · ${d.travelTimeTotalHr}h moving` : '');
      return `${head}\n${stops}`;
    };
    return [
      plan.tripTitle || title,
      [plan.dailyPlan?.length ? `${plan.dailyPlan.length} day${plan.dailyPlan.length === 1 ? '' : 's'}` : null, plan.estimatedBudgetInr ? `about ₹${Number(plan.estimatedBudgetInr).toLocaleString('en-IN')}` : null].filter(Boolean).join(' · '),
      '',
  ...
  const sharePlan = async () => {
    ...
    try {
      ...
      else { await navigator.clipboard?.writeText(url ? `${text}\n${url}` : text); setError('Plan copied — paste it anywhere'); }
    } catch (e) { if (e?.name !== 'AbortError') setError('Could not share'); }
  };
  // Same plan as a PDF: the share sheet on a phone, a download on a desktop.
  const sharePdf = async () => {
    setPdfBusy(true);
    try { const how = await api.exportSavePdf(id, plan?.tripTitle || title); if (how === 'downloaded') setError('PDF downloaded'); }
    catch (e) { setError(e.message || 'Could not make the PDF'); }
    ...
    <div className="wt-screen">
      ...
      {plan && !building && (
        <div style={{ marginTop: 'auto', paddingTop: 16, display: 'flex', gap: 10 }}>
          ...
          <Button small variant="secondary" icon="book" onClick={sharePdf} disabled={pdfBusy} style={{ width: 'auto', padding: '0 16px' }}>{pdfBusy ? 'Making PDF…' : 'Share as PDF'}</Button>
