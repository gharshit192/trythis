...
// ─── Main ─────────────────────────────────────────────────────────────────────
const relativeTimeLower = (d) => relativeTime(d).toLowerCase();
...
export default function SaveDetail({ onNavigate, onBack, payload }) {
  ...
  const isTravel = ['travel', 'experience', 'hotel'].includes(save?.category);
  ...
  return (
    <div className="phone-frame" style={{ background: T.bg, color: T.text, minHeight: '100vh' }}>
      <div style={{ display: 'flex', flexDirection: 'column', padding: '0 0 80px' }}>
        {/* Header — no hero image; the saved photo lives in the source row below (ADR 0013) */}
        <div style={{ padding: 'var(--pad-top) var(--pad-screen) 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 26 }}>
            <button type="button" onClick={onBack || (() => onNavigate('home'))} aria-label="Back" style={{ background: 'none', border: 0, cursor: 'pointer', color: 'var(--ink)' }}><Icon name="back" size={22} /></button>
            <div style={{ display: 'flex', gap: 18 }}>
              <button type="button" onClick={handleShare} aria-label="Share" style={{ background: 'none', border: 0, cursor: 'pointer', color: 'var(--ink)' }}><Icon name="share" size={21} /></button>
              <button type="button" onClick={() => setConfirmDelete(true)} aria-label="More" style={{ background: 'none', border: 0, cursor: 'pointer', color: 'var(--ink)' }}><Icon name="more" size={21} /></button>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <span style={{ width: 8, height: 8, borderRadius: 4, background: `var(--cat-${tile.kind})` }} />
            <span className="wt-eyebrow" style={{ fontSize: 12, letterSpacing: '.1em' }}>{tile.label}</span>
            {(save?.extractedLocation?.name || save?.extractedLocation?.city) && (
              <span style={{ fontSize: 12, color: 'var(--faint)' }}>· {[save?.extractedLocation?.name, save?.extractedLocation?.city].filter(Boolean).join(', ')}</span>
            )}
          </div>
          <h1 className="wt-title lg" style={{ marginBottom: 10 }}>{safeTitle}</h1>
          <div style={{ fontSize: 14.5, color: 'var(--mute)', marginBottom: 22, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <span>Saved {relativeTimeLower(save?.createdAt)}{save?.source ? ` · ${save.source[0].toUpperCase() + save.source.slice(1)}` : ''}</span>
            {save?.processingStatus && save.processingStatus !== 'completed' && (
              <span style={{ padding: '3px 9px', borderRadius: 999, fontSize: 11.5, fontWeight: 600, background: sm.bg, color: sm.fg }}>{sm.label}</span>
            )}
          </div>
          <div style={{ marginBottom: 24 }}>
            <StatusControl
              value={save?.intentStatus || 'saved'}
              onChange={(next) => next === 'tried'
                ? onNavigate('tried', { id, title: safeTitle, createdAt: save?.createdAt })
                : handleIntent(next)}
            />
          </div>
        </div>
                  ...
                  {recipe.steps.map((s, i) => (
                    <li key={i} style={{ display: 'flex', gap: 12, marginBottom: i < recipe.steps.length - 1 ? 12 : 0, alignItems: 'flex-start' }}>
                      <span style={{
                        ...
                        background: `var(--cat-${tile.kind})`, color: '#fff',
