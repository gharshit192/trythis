...
/**
 * Analyze a user's saving patterns and derive a persona.
 *
 * Improvements over v1:
 *   - Requires concentration (>40% of saves) to claim a persona
 *   - Adds 'multi_interest' bucket for genuinely diverse users
 *   - Distinguishes 'new_user', 'casual', and engaged personas
 *   - Captures recency-weighted preferences (recent saves matter more)
 */
 ...
 *
 * Budget is per-rolling-24h, not per-calendar-day.
 */
function getNotificationBudget(persona) {
  const { saveCount = 0, confidence = 0 } = persona || {};

  // New users: be quiet. Build trust first.
  if (saveCount < 10) return 1;

  // Engaged users: 2/day max
  if (saveCount < 50) return 2;

  // Power users with clear preferences: up to 3/day
  // (Only get the +1 if persona is well-defined)
